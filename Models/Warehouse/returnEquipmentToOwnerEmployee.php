<?php
	...
	try{
		...
		$result->execute([":ID" => POST("warehousedetID"), ":no" => $_SESSION["SUPMIS"]["supmis_ID"]]);	   	 	

	    if($result->rowCount() == 0 || $result->rowCount() == "0")
	    	throw new Exception("Update Failed");

		$result = $db->prepare("INSERT INTO `tblreturn`(`reissueID`, `isReturn`, `returnDate`, `returnIsWork`, `returnRemarks`, `officerID`, `no`) SELECT `reissueID`, 0, CURDATE(), `returnIsWork`, `returnRemarks`, :Officer, :no  FROM `tblreturn`  WHERE `returnID` = :ID");
		$result->execute([":ID" => POST("returnID"), ":Officer" => $_SESSION["SUPMIS"]["supmis_ID"], ":no" => $_SESSION["SUPMIS"]["supmis_ID"]]);	   	 	
		...
		$result->execute([":ID" => POST("returnID")]);
		$row = $result->fetch();
		...
		$LogRemarks = $LogsSupply . " has been returned to of the Mr/Ms. " . $LogsEmpName . " (". $LogsProperty .")";
	    ...
	    $db->commit();
	    $arrayReturnEquipment[] = array("message" => "success");
	}
	catch(Exception $e){
	    $arrayReturnEquipment = [];
    	$arrayReturnEquipment[] = array("message" => "Connection Error", "error" => $e, "err" => $e->getMessage());
	    $db->rollBack();
	}

	jsEncode($arrayReturnEquipment);
